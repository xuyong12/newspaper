...
public class tongji3 extends JFrame {
	...
	public tongji3() {
		...
		try {
			model.setDataVector(new view_3().all(), header);
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		// String[] header ={
		// "用户账号","用户密码","用户名","用户性别","用户地址","用户电话","用户邮箱","用户部门号","用户订阅数量" };
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (name.equals("")) {
					...
					if (view.compare(name)) {
						...
						int j;
						for (j = 0; j < y; j++) {
							String str = (String) table.getValueAt(j, 3);
		...
		button.setBounds(535, 129, 86, 33);
		...
		button_1.setBounds(680, 129, 86, 33);
